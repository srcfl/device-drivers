...
### Fixed
- **`sigenergy` 1.1.3 — a plant with no storage reported a battery sitting at 0%.** The two ESS registers (30014 SOC, 30037 power) are the only ones this driver reads that a Sigenergy plant can lack: every hybrid model in the range is sold without storage as well as with it, and on such a plant neither register answers. `probe_read` already handled that correctly, returning `nil` after three tries and then leaving the register alone — but the emit below it ignored the result, defaulted both fields to zero and published them anyway. Zero is not "no battery": it is an empty pack that can absorb charge, and it is indistinguishable from a real one that has just run flat, so nothing downstream could tell the two apart. The fields are now filled only from registers that answered, and the DER is emitted only when at least one did — the shape `ambibox` uses for its own conditional battery, with the per-field guards `pixii` and `ferroamp_modbus` use. A plant with storage is unaffected
- **Found on a Sigen PV 50M1-HYA commissioned PV-only.** Table 2-1 of the Sigenergy Modbus Protocol (V2.9) files that model under **`Hybrid Inv.`**, not `PV Inv.` — plain `Sigen PV 50M1` without the `-HYA` suffix is the `PV Inv.` entry — so every register this driver touches is one the model implements, and the PV, grid-sensor and PV-curtailment paths all work on it. Only the ESS half is silent, which is what made the phantom visible: the driver was otherwise entirely healthy on that hardware. `ders` is deliberately unchanged — the driver does produce a battery on a plant that has one, and the manifest describes the driver rather than any one site
- **The control path is left for a separate change.** `driver_command("battery", …)` still writes 40031/40032/40034 on a plant with no ESS, where 40032 and 40034 are not implemented. That is the same root cause, but control changes carry their own review and hardware bar here, and this fix is what stops the planner being told there is a battery to dispatch in the first place
