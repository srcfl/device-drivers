...
-- Lua caught it, so a register this meter does not answer -- a PAC2200 does
-- not carry every PAC3200 block -- costs a failed read on every poll forever
-- and the stale-telemetry watchdog takes the site offline. Ask three times,
-- since one miss can just be a slow link, then leave the register alone. A
-- restart re-probes.
...
local function probe_read(addr, count, kind)
    ...
    if failures == GIVE_UP_AFTER then
        host.log("info", string.format(
            "Siemens: register %d did not answer %d times; leaving it alone " ..
...
function driver_poll()
    ...
    local v_regs = probe_read(1, 6, "holding")
    ...
    local a_regs = probe_read(13, 6, "holding")
    ...
    local w_regs = probe_read(25, 6, "holding")
    ...
    local hz_regs = probe_read(55, 2, "holding")
    local hz = 0
    if hz_regs then
    ...
    local tw_regs = probe_read(65, 2, "holding")
    local total_w = 0
    if tw_regs then
    ...
    local imp_regs = probe_read(801, 2, "holding")
    local import_wh = 0
    if imp_regs then
    ...
    local exp_regs = probe_read(809, 2, "holding")
    local export_wh = 0
    if exp_regs then
