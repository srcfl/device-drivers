...
function driver_poll()
    ...
    local l1_w = 0
    if ok_l1w then l1_w = host.decode_f32_be(l1w_regs[1], l1w_regs[2]) end

    local ok_l2w, l2w_regs = pcall(host.modbus_read, 40102, 2, "holding")
    local l2_w = 0
    if ok_l2w then l2_w = host.decode_f32_be(l2w_regs[1], l2w_regs[2]) end

    local ok_l3w, l3w_regs = pcall(host.modbus_read, 40104, 2, "holding")
    local l3_w = 0
    if ok_l3w then l3_w = host.decode_f32_be(l3w_regs[1], l3w_regs[2]) end

    -- Export energy: 40130-40131, F32 BE, Wh
    local ok_exp, exp_regs = pcall(host.modbus_read, 40130, 2, "holding")
    local export_wh = 0
    if ok_exp then
        export_wh = host.decode_f32_be(exp_regs[1], exp_regs[2])
    end
    ...
    local import_wh = 0
    if ok_imp then
        import_wh = host.decode_f32_be(imp_regs[1], imp_regs[2])
    end
