...
function driver_poll()
    ...
    local pv_w = 0
    if ok_pvw then
        pv_w = host.decode_i32_be(pvw_regs[1], pvw_regs[2])
    end
    ...
    local mppt1_a = 0
    if ok_m1a then
        mppt1_a = host.decode_i32_be(m1a_regs[1], m1a_regs[2]) * 0.001
    end
    ...
    local mppt1_v = 0
    if ok_m1v then
        mppt1_v = host.decode_i32_be(m1v_regs[1], m1v_regs[2]) * 0.01
    end
    ...
    local mppt2_a = 0
    if ok_m2a then
        mppt2_a = host.decode_i32_be(m2a_regs[1], m2a_regs[2]) * 0.001
    end
    ...
    local mppt2_v = 0
    if ok_m2v then
        mppt2_v = host.decode_i32_be(m2v_regs[1], m2v_regs[2]) * 0.01
    end
    ...
    local inv_temp = 0
    if ok_itemp then
        inv_temp = host.decode_i32_be(itemp_regs[1], itemp_regs[2]) * 0.1
    end
    ...
    local bat_a = 0
    if ok_ba then
        bat_a = host.decode_i32_be(ba_regs[1], ba_regs[2]) * 0.001
    end
    ...
    local bat_soc = 0
    if ok_bsoc then
        bat_soc = host.decode_u32_be(bsoc_regs[1], bsoc_regs[2]) / 100  -- to fraction
    end
    ...
    local bat_temp = 0
    if ok_btemp then
        bat_temp = host.decode_i32_be(btemp_regs[1], btemp_regs[2]) * 0.1
    end
    ...
    local bat_v = 0
    if ok_bv then
        bat_v = host.decode_u32_be(bv_regs[1], bv_regs[2]) * 0.01
    end
    ...
    local meter_w = 0
    if ok_mw then
        meter_w = host.decode_u32_be(mw_regs[1], mw_regs[2])
    end
    ...
    if ok_mpw then
        l1_w = host.decode_u32_be(mpw_regs[1], mpw_regs[2])
        l2_w = host.decode_u32_be(mpw_regs[3], mpw_regs[4])
        l3_w = host.decode_u32_be(mpw_regs[5], mpw_regs[6])
    end
    ...
    if ok_lv then
        l1_v = host.decode_u32_be(lv_regs[1], lv_regs[2]) * 0.01
        l2_v = host.decode_u32_be(lv_regs[3], lv_regs[4]) * 0.01
        l3_v = host.decode_u32_be(lv_regs[5], lv_regs[6]) * 0.01
    end
    ...
    if ok_la then
        l1_a = host.decode_u32_be(la_regs[1], la_regs[2]) * 0.001
        l2_a = host.decode_u32_be(la_regs[3], la_regs[4]) * 0.001
        l3_a = host.decode_u32_be(la_regs[5], la_regs[6]) * 0.001
    end
