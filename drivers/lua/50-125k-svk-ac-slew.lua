-- Solis S6 Hybrid 50~125 kW — AC-PORT control variant w/ slew capability.
-- Version: 0.2.3
-- 0.2.2 → 0.2.3: two fixes. (1) self-heal now compares the AC grid-port
--                readback (ac_w, reg 34765) — not the DC battery
--                readback (bat_w, reg 34755) — against the AC-committed
--                setpoint, guarded on ac_w ~= nil; on a hybrid inverter
--                DC ≠ AC by house load + losses so the old bat_w
--                comparison spuriously tripped self-heal. (2)
--                deinit_safe_revert resets slew_last_write_ms = now_ms()
--                instead of the undeclared global slew_last_write_poll.
--
-- Forked from `50-125k-svk-slew@0.1.7`. The ONLY architectural change
-- is that this driver writes to the Solis **AC Grid Port** registers
-- (44280 BIT00-03 = 2, 44284-85 power value, +export/-import) instead
-- of the **Battery Port** (44280 = 4, 44282-83, +charge/-discharge).
--
-- ── WHY AN AC-PORT CONTROL DRIVER ────────────────────────────────────
-- For SvK FCR-D the scorer evaluates **grid-facing** active power
-- (Acuvim total system power, what the public grid actually sees).
-- Battery-port control commands battery DC power; that converts to
-- AC through the hybrid inverter MINUS the house load on the same AC
-- bus. House load drift during a 40-s deactivation window biases
-- §3.1.2 Req 4 (deactivation energy ≤ 2.5 s × ΔP_ss,theo) — a 200 W
-- step in the kitchen during ramp 6 falsely registers as 8 kWs of
-- excess inverter delivery against a 25 kWs cap.
--
-- Commanding the AC port directly closes the loop where the metric
-- measures it: the inverter compensates internally for house load so
-- the grid sees exactly the commanded power. The same scorer + same
-- Acuvim reading then evaluates a clean signal.
--
-- ── SIGN CONVENTION (carefully) ──────────────────────────────────────
-- Sourceful (controller side, unchanged across drivers):
--   +W = charge battery (= import from grid)
--   -W = discharge battery (= export to grid)
-- Solis Battery reg 44282-83: same convention as Sourceful — no flip.
-- Solis AC reg 44284-85: **OPPOSITE** sign — +W = export, -W = import.
-- This driver negates `value` inside `write_ac_s32` at the bus
-- boundary so the controller, the slew block, and the arbitrator
-- handle the same sp_w semantics no matter which port is active.
--
...
--   44284-44285  Power Control AC Power Value, signed int32, 1 W.
--          Solis convention +export to grid / -import from grid;
--          driver flips sign at the wire boundary (write_ac_s32) so
--          the controller-side sp_w stays in Sourceful convention.
...
DRIVER = {
    host_api_min = 1,
    host_api_max = 1,
    id = "50-125k-svk-ac-slew",
    name = "Solis S6 50-125 kW C&I hybrid (SvK, AC-stage slew-limited)",
    manufacturer = "Solis",
    version = "0.2.3",
    protocols = { "modbus" },
    capabilities = { "battery" },
    read_only = false,
    description = "Solis 50-125 kW three-phase C&I hybrid via Modbus RTU; SvK fast-frequency control on the AC power register with configurable deactivation slew.",
    ...
    options = {
        { name    = "battery_rated_w",
          ...
          help    = "Battery max continuous charge / discharge power (W, " ..
                    ...
                    "before writing the 44284-85 AC power value." },
...
local ARM_AC_GRID = 0x0002

-- Slew state: target = what the controller asked for; committed =
-- what we last wrote to 44284; last_write_ms = host.now_ms() at that
...
-- Write a signed 32-bit power value to the Solis AC Grid Port
-- power register (44284-85). Sign convention crosses here:
--   * Sourceful (controller-side): +W = charge battery / import from
--     grid, -W = discharge battery / export to grid. This matches the
--     Sourceful battery sign convention, so the upstream arbitrator
--     and slew logic produces the SAME sp_w regardless of whether
--     the active driver controls battery (44282) or AC grid (44284).
--   * Solis AC reg 44284-85: +W = export to grid, -W = import from
--     grid. The sign is flipped relative to Sourceful's convention.
-- We negate `value` at the bus boundary so the controller and the
-- battery-port slew driver share the same internal sign semantics —
-- only the wire-format byte differs between drivers.
local function write_ac_s32(label, base_addr, value)
    local wire = -value  -- Sourceful → Solis AC sign flip
    if wire < 0 then wire = wire + 4294967296 end
    wire = wire % 4294967296
    local hi = math.floor(wire / 65536)
    local lo = wire % 65536
    local ok = host_write_ok(host.write_registers, base_addr, { hi, lo })
    if ok then
        host.log(string.format(
            "[%s] FC16 reg %d..%d = %d (Sourceful) → %d (Solis AC wire, hi=%d lo=%d)",
            label, base_addr, base_addr + 1, value, -value, hi, lo))
...
local function arm_remote_mode()
    ...
    local ok = write_seq("init", { { 44280, ARM_AC_GRID } })
    if ok then
        ok = write_ac_s32("init", 44284, 0)
    end
...
function driver_poll()
    ...
    if control_initialized and slew_committed_w ~= slew_target_w then
        ...
        if next_w ~= slew_committed_w then
            ...
            if write_seq("slew", { { 44280, ARM_AC_GRID } }) then
                if write_ac_s32("slew", 44284, next_w) then
    ...
    -- AC-PORT variant: self-heal must compare the **AC grid-port**
    -- readback (ac_w, reg 34765) against committed_w — NOT the DC
    -- battery readback (bat_w, reg 34755). On a hybrid inverter the DC
    -- battery power differs from AC grid power by house load + losses,
    -- so comparing bat_w would spuriously fire self-heal on every
    -- faithfully-held AC setpoint. write_ac_s32 negates value at the
    -- wire (Sourceful +import → Solis AC -import), so a held AC
    -- setpoint reads back as ac_w ≈ -slew_committed_w. Skip the check
    -- when ac_w is nil (AC readback unavailable this poll).
    if control_initialized and ac_w ~= nil then
        local err_w = math.abs(ac_w - (-slew_committed_w))
        ...
        if err_w > tol_w
           ...
           and slew_committed_w == slew_target_w then
            ...
            if self_heal_disagree_polls >= SELF_HEAL_RUN_REQUIRED then
                host.log(string.format(
                    "self-heal: committed=%dW but ac_w=%dW for %d polls — re-arm + rewrite",
                    slew_committed_w, ac_w, self_heal_disagree_polls))
                if write_seq("self-heal", { { 44280, ARM_AC_GRID } }) then
                    write_ac_s32("self-heal", 44284, slew_committed_w)
                    slew_last_write_ms = now_ms()
                end
                self_heal_disagree_polls = 0
            end
...
local function deinit_safe_revert()
    ...
    slew_last_write_ms = now_ms()
    self_heal_disagree_polls = 0
    local ok1 = write_ac_s32("deinit", 44284, 0)
...
function driver_command(action, power_w, ctx)
    ...
    if action == "battery" then
        ...
        if mag_target >= mag_committed then
            ...
            host.log(string.format(
                "CMD: %s %dW (activation/hold, slew bypassed) → 44284-85=%d",
                dir, target_w, target_w))
            if not write_seq("battery", { { 44280, ARM_AC_GRID } }) then
                host.log("CMD FAIL: could not re-assert arm 44280")
                return false
            end
            local ok = write_ac_s32("battery", 44284, target_w)
            if ok then
            ...
            -- earlier version reset it on every deact command, which
            -- starved the slew entirely under a continuously-falling
            -- setpoint (arbitrator dispatches every ~16 ms mid-flank
            -- → elapsed pinned ≈0 → budget ≈0 → no writes, then one
            -- cliff-snap when the flank levels off — observed live
            -- 2026-06-10 on the battery-port twin). Post-activation
            -- budget accumulation is already bounded by
            -- MAX_BUDGET_ELAPSED_MS (1 s = SvK measurement window).
            host.log(string.format(
