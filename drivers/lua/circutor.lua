...
-- read once the meter has made clear it will not answer — otherwise the
-- poll keeps failing, the stale-telemetry watchdog marks the meter offline,
-- and the site reports nothing at all instead of one field less. The CEM-C
-- and CVM families differ in which blocks they populate, so this matters
-- here.
--
...
local function probe_read(addr, count, kind)
    ...
    if failures == GIVE_UP_AFTER then
        host.log("info", string.format(
            "Circutor: register %d did not answer %d times; leaving it " ..
...
function driver_poll()
    ...
    local tw_regs = probe_read(18, 2, "holding")
    local total_w = 0
    if tw_regs then
    ...
    local hz_regs = probe_read(36, 2, "holding")
    local hz = 0
    if hz_regs then
    ...
    local imp_regs = probe_read(40, 2, "holding")
    local import_wh = 0
    if imp_regs then
    ...
    local exp_regs = probe_read(42, 2, "holding")
    local export_wh = 0
    if exp_regs then
