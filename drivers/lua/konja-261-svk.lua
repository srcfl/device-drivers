...
local function write_setpoint_konja_w(label, konja_w)
    ...
    local ok = host_write_ok(host.write_registers, R.SLAVE_POWER, { hi, lo })
    if ok then
        host.log(string.format(
...
local function verify_setpoint_encoding()
    ...
    local ok_w = host_write_ok(host.write_registers, R.SLAVE_POWER, { want_hi, want_lo })
    if not ok_w then
    ...
    host.log(string.format("encoding self-test: read back hi=0x%04X lo=0x%04X",
                           rr[1], rr[2]))
    host_write_ok(host.write_registers, R.SLAVE_POWER, { 0, 0 })   -- always leave at 0
...
-- the EMS refuses the 0 W write there, keep asking a bounded number of
-- times and then stop reissuing it unprompted — the EMS's own 3-minute
-- comms watchdog is the independent fail-safe (see deinit_safe_revert),
-- and a write the device has refused three times in a row will not land
-- on the fourth. An explicit "deinit" command always attempts the revert
-- regardless (that path is prompted), and the counter resets on any
-- successful write.
function driver_default_mode()
    ...
    if ok then
        ...
        if default_mode_refusals == DEFAULT_MODE_REFUSALS_BEFORE_STOP then
            host.log("default_mode: EMS refused the 0 W revert "
                .. default_mode_refusals .. " times in a row; not reissuing it "
                .. "unprompted (EMS comms watchdog forces 0 W independently)")
        end
    end
    return ok
end
